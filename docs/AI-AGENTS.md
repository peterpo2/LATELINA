...
## Overview
Latelina ships with an OpenAI-powered assistant that answers medical questions and guides users through the storefront. This document explains how the existing integration works and how to launch additional automation agents for development or support workflows.

## Existing assistant
- **Endpoint:** `Latelina.Backend/Latelina.Web` exposes the chat API that the React widget calls.
...
2. Use the following starter prompts when bootstrapping a development-focused agent:
   - **English:** `Act as the Latelina development co-pilot. Before writing code, list the affected UI routes, check the current release version (0.1.0), and outline a step-by-step plan with test coverage ideas.`
   - **Български:** `Действай като Latelina агент за разработка. Провери текущата версия (0.1.0), изброи засегнатите страници и предложи план за имплементация и тестове.`
